...
# Ensure the writable .ssh directory exists (from emptyDir mount)
mkdir -p "$AUTHORIZED_KEYS_DIR"
chmod 700 "$AUTHORIZED_KEYS_DIR"

# Copy public key(s) from the prepared /config/ssh/keys into authorized_keys
if [ -f /config/ssh/keys/id_ed25519.pub ]; then
  cat /config/ssh/keys/id_ed25519.pub >> "$AUTHORIZED_KEYS_FILE"
  chmod 600 "$AUTHORIZED_KEYS_FILE"
fi

# Enforce key-only auth if PASSWORD_ACCESS is "false"
